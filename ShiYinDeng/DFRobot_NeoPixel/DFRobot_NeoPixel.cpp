...
#if defined(ARDUINO_ARCH_RP2040)
void DFRobot_NeoPixel::rp2040Init(uint8_t pin, bool is800KHz)
{
...
// Not a user API
void  DFRobot_NeoPixel::rp2040Show(uint8_t pin, uint8_t *pixels, uint32_t numBytes, bool is800KHz)
{
  if (this->init)
  {
...
void DFRobot_NeoPixel::showBarGraph(uint16_t start, uint16_t end, int16_t value, int16_t high)
{
    ...
    start = min((int)start, (int)numLEDs);
    end = max((int)end, 0);
    end = min((int)end, (int)numLEDs);
#endif
...
uint32_t DFRobot_NeoPixel::hsl(uint32_t h, uint32_t s, uint32_t l)
{
    ...
    l = max((int)l, 0);
    l = min((int)l, 99);
    uint32_t c = (((100 - abs(2 * (int)l - 100)) * s) << 8) / 10000; //chroma, [0,255]
    uint8_t h1 = h / 60;                                        //[0,6]
